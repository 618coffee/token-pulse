...
# token-meter — Token Usage Attribution

## When to use this skill

Invoke `token-meter` when the user asks any of:
...
`token-meter` is a CLI. Run it via the terminal. Subcommands map directly to scopes:

| User intent | Command |
|---|---|
| Last assistant turn | `token-meter turn` |
| Current session | `token-meter session` |
| A specific commit | `token-meter commit <sha>` |
| Range of commits | `token-meter commit <from>..<to>` |
| Since branching off main | `token-meter branch --base main` |
| A pull request | `token-meter pr <number>` |
| Custom time window | `token-meter window --since <iso> --until <iso>` |
| Per-file attribution | `token-meter file <path>` or `token-meter file --top 10` |
| Machine-readable output | append `--json` |

### Recommended workflow

1. **Confirm install**: `token-meter --version`. If missing, suggest `pipx install token-meter`.
2. **Pick the right scope**:
   - If the user mentions a SHA, branch name, or PR number → use that scope directly.
   - If they say "this commit" / "the last commit" → `token-meter commit HEAD`.
   - If they say "today" / "this morning" → `token-meter window --since <iso>`.
   - If they say "this session" / "this conversation" → `token-meter session`.
...
token-meter commit HEAD~5..HEAD --rank

# "Which file in this repo eats the most context?"
token-meter file --top 10
...
`token-meter` reports four token classes:
...
The "Estimated cost" line uses built-in pricing (overridable via `~/.config/token-meter/config.toml`). Always present cost as **estimated** unless the user is on a metered API key for which the model+pricing is known-current.
