...
# tokmeter — Token Usage Attribution

## When to use this skill

Invoke `tokmeter` when the user asks any of:
...
`tokmeter` is a CLI. Run it via the terminal. Subcommands map directly to scopes:

| User intent | Command |
|---|---|
| Last assistant turn | `tokmeter turn` |
| Current session | `tokmeter session` |
| A specific commit | `tokmeter commit <sha>` |
| Range of commits | `tokmeter commit <from>..<to>` |
| Since branching off main | `tokmeter branch --base main` |
| A pull request | `tokmeter pr <number>` |
| Custom time window | `tokmeter window --since <iso> --until <iso>` |
| Per-file attribution | `tokmeter file <path>` or `tokmeter file --top 10` |
| Machine-readable output | append `--json` |

### Recommended workflow

1. **Confirm install**: `tokmeter --version`. If missing, suggest `pipx install tokmeter`.
2. **Pick the right scope**:
   - If the user mentions a SHA, branch name, or PR number → use that scope directly.
   - If they say "this commit" / "the last commit" → `tokmeter commit HEAD`.
   - If they say "today" / "this morning" → `tokmeter window --since <iso>`.
   - If they say "this session" / "this conversation" → `tokmeter session`.
...
tokmeter commit HEAD~5..HEAD --rank

# "Which file in this repo eats the most context?"
tokmeter file --top 10
...
`tokmeter` reports four token classes:
...
The "Estimated cost" line uses built-in pricing (overridable via `~/.config/tokmeter/config.toml`). Always present cost as **estimated** unless the user is on a metered API key for which the model+pricing is known-current.
